...
class SetDirectory {
  constructor(option) {
    ...
    let leaveSecond = (directory, index) => {
      if (directory[index].childDir.length != 0) {
        directory[index].childDir[
          this.secondDir[index].currIndex
        ].dirActive = false;
      }
    };
    // 进入二级目录
    let enterSecond = (directory, index) => {
      if (
        this.secondDir[index].currIndex &&
        directory[index].childDir.length != 0
      ) {
        directory[index].childDir[
          this.secondDir[index].currIndex
        ].dirActive = true;
  ...
  scrollToHash(offset) {
    ...
    const offsetTop =
      offset ||
      (this.route.hash &&
        document.querySelector(this.route.hash).offsetTop - 80);
    ...
    const fn = () => {
      ...
      if (start < offsetTop) {
        this.container.scrollTop =
          scrollTop + isspeed > offsetTop ? offsetTop : scrollTop + isspeed;
  ...
  setDirNode() {
    ...
    const directory = dirNode.map((item) => {
      ...
      if (this.secondDir.contentNode) {
        setArray(
          item.parentNode.getElementsByTagName(this.secondDir.contentNode)
        ).map((node, index) => {
          childDir.push({
            name: node.innerText,
  ...
  scrollDir(e, dir = this.directory.value, startDir) {
    ...
    if (e.target.scrollTop > dir[currIndex].offsetTop - 100) {
      ...
      // 滚动二级目录
      dir[currIndex].childDir &&
        this.scrollDir(e, dir[currIndex].childDir, this.secondDir[currIndex]);
      // 滚动到下一个目录
      if (
        currIndex < dir.length - 1 &&
        e.target.scrollTop > dir[currIndex + 1].offsetTop - 100
      ) {
