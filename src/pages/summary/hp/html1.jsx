        ...
        return ()=>(
            <div class={'hp-container'}>
                ...
                <div class='web-content' >
                    <h3>含义示例</h3>
                    <p>&nbsp;&nbsp;  HTTP（Hypertext Transfer Protocol）是一种用于传输超文本的协议,用来在客户端和服务器之间传输数据。前端开发人员需要了解HTTP请求和响应的基本原理、常见的请求方法（GET、POST等）以及状态码的含义。</p>
                    <br />
                    <p>它是一种被广泛应用在浏览器客户端中的应用层协议，它定义了客户端与服务端之间进行数据交互的一组规则，在和服务端建立连接后，通过携带请求消息，请求体发送数据请求。</p>
                    <br />
                    <p>&nbsp;&nbsp;&nbsp;我们通常是使用浏览器通过输入http协议的URL来访问 Web 服务器，事实上，浏览器并不只有这一个功能，它也可以用来在 FTP服务器上下载和上传文件，同时也具备电子邮件客户端的功能。可以说，浏览器是一个具备多种客户端功能的综合性客户端软件，因此它需要
                        一些东西来判断应该使用其中哪种功能来访问相应的数据，而各种不同的
                        URL 就是用来干这个的，比如访问 Web 服务器时用“http:”，而访问FTP服务器时用“ftp:”</p>
                </div>
                <h1>URL</h1>
                <div className="web-content">
                    ...
                    <br />
                    <p>
                        让我们来介绍一下网址。网址，准确来说应该叫URL(统一资源定位符),是用于标识和定位网络上资源的地址,它以http://开头，除了http，还有其他类型的网址，例如 "ftp","file","maito"等。
                        一般的话，一个完整的URL包括4个部分：协议类型(访问方法)、域名、端口号(可省略)、服务器目录文件路径；以百度网址为例：
                    </p>
                    <br />
                    <code><i>http://www.baidu.com:80/dir/file.html</i></code>
                    <br />
                    <p>这表示要访问百度web服务器上路径名wei/dir/file.html的文件，也就是位于/dir/目录下的file.html这个文件</p>
                ...
                <div className="web-content">
                    <h3>含义示例</h3>
                    <p>
                        DNS(Domain Name System),域名服务系统。将服务器名称和IP地址进行关联是DNS最常见的用法，但DNS的功能并不仅限于此，他还可以将邮件地址和邮件服务器进关联，以及为各种信息关联相印的名称
                ...
                <div className="web-content">
                    <h3>概述</h3>
                    <p>
                        在 HTML 中，<code>{"<a>, <form>, <img>, <script>, <iframe>, <link>"}</code> 等标签以及 Ajax 都可以指向一个资源地址，跨域请求就是指：当前发起请求的域与该请求指向的资源所在的域不一样，凡是发送请求的url的 协议、域名、端口号三者之间任意一者与当前页面地址不同的请求。这里的域指的是这样的一个概念：我们认为若协议 + 域名 + 端口号均相同，那么就是同域
                    </p>
                    <p>
                ...
                <div className="web-content">
                    ...
                    <p>
                        ...
                        <span>
                            3. 现在所有的可支持 Javascript 的浏览器都会使用这个策略
                        </span>

                        <span>4. web构建在同源策略基础之上，浏览器对非同源脚本的限制措施是对同源策略的具体实现</span>
                    </p>
